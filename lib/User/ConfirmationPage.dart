...
class Confirmation extends StatefulWidget {
  ...
  @override
  _ConfirmationState createState() => _ConfirmationState(selecteditems,date,pgname);
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:Container(
        color: Colors.white,
        child: StreamBuilder(
            stream: Firestore.instance.collection('/pgs/damana/$date/').orderBy("index").snapshots(),builder: (context, snapshot){
          ...
          return  ListView.builder(itemCount: selecteditems.length,
              itemBuilder: (BuildContext context,int index){
                return ListTile(

                    title: Text(" الساعات المحجوزةبتاريخ  "  +date.toString()+" $pgnameملعب"+snapshot.data.documents[selecteditems[index]]["price"].toString()),
